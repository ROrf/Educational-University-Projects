...
def assignmentAverage(element):
    assignmentAverage = 0.0
    numberOfEntries = 0
    for row in gradebook:
        assignmentAverage += row[element]
        numberOfEntries += 1
    assignmentAverage /= numberOfEntries
    return assignmentAverage

def studentAverage(row):
    studentAverage = 0.0
    for element in row:
        studentAverage += element
    studentAverage /= len(row)
    return studentAverage

def printList(averageList, printCondition):
    number = 1
    if(printCondition == 0):
        print("\nAssignment Averages:")
        for x in averageList:
            print("Assignment %d" % number + ": %.2f" % x)
            number += 1
    else:
        print("\nStudent Averages:")
        for x in averageList:
            print("Student %d" % number + ": %.2f" % x)
            number += 1

def main():
    assignmentAverages = []
    studentAverages = []
    assignmentAverages = [assignmentAverage(element) for element in range(0,len(gradebook[0]))]
    studentAverages = [studentAverage(row) for row in gradebook]
    printList(assignmentAverages, 0)
    printList(studentAverages, 1)
